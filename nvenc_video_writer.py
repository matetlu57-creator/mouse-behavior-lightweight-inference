...
class NVENCWriter:
    def __init__(self, path: str | Path, fps: float, width: int, height: int, cfg: Optional[Mapping[str, Any]] = None):
        cfg = dict(cfg or {})
        ffmpeg = shutil.which("ffmpeg")
        ...
        cmd = [
            ...
            "-s", f"{self.width}x{self.height}", "-r", f"{float(fps):.12g}",
            "-i", "-", "-an", "-c:v", codec, "-preset", preset,
            "-rc", "vbr", "-cq", cq, "-pix_fmt", "yuv420p", self.path,
        ]
    ...
    def write(self, frame: np.ndarray) -> None:
        ...
        if arr.shape[:2] != (self.height, self.width) or arr.ndim != 3 or arr.shape[2] != 3:
            raise ValueError(f"unexpected frame shape {arr.shape}; expected {(self.height, self.width, 3)}")
        try:
...
class OpenCVWriter:
    def __init__(self, path: str | Path, fps: float, width: int, height: int, fourcc: str = "mp4v"):
        self.writer = cv2.VideoWriter(str(path), cv2.VideoWriter_fourcc(*fourcc), float(fps), (int(width), int(height)))
    def isOpened(self) -> bool:
...
def create_video_writer(path: str | Path, fps: float, width: int, height: int, cfg: Optional[Mapping[str, Any]] = None):
    cfg = dict(cfg or {})
