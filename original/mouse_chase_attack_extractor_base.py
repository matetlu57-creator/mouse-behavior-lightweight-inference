...
class Detection:
    ...
    def center_px(self) -> np.ndarray:
        ...
        if n > 0:
            ...
            core_idx = [idx for idx in (KP["neck"], KP["left_hind"], KP["right_hind"], KP["tail"]) if idx < n]
...
class ObservationHistory:
    ...
    def add(self, observation: MouseObservation) -> None:
        self.data[int(observation.logical_id)].append(observation)
...
class PairFeatureComputer:
    ...
    def _trajectory_features(
        ...
        actor_by_frame = {o.frame: o for o in actor_hist}
        target_by_frame = {o.frame: o for o in target_hist}
        ...
        valid = np.all(np.isfinite(actor_centers), axis=1) & np.all(np.isfinite(target_centers), axis=1)
        ...
        actor_path = float(np.sum(np.linalg.norm(da, axis=1)))
        target_path = float(np.sum(np.linalg.norm(dt, axis=1)))
        return corr, actor_path, target_path
    ...
    def _distance_drop(
        ...
        if not np.isfinite(previous_distance):
            return 0.0
