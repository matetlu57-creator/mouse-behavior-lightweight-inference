...
def load_modules():
    ...
    new_base = load_module(
        "base_optimized_for_test", ROOT / "mouse_chase_attack_extractor_base.py"
    )
    sys.modules["mouse_chase_attack_extractor_base"] = old_base
    old_main = load_module(
        "main_original_for_test", ORIGINAL / "mouse_chase_attack_high_recall.py"
    )
    sys.modules["mouse_chase_attack_extractor_base"] = new_base
    new_main = load_module(
        "main_optimized_for_test", ROOT / "mouse_chase_attack_high_recall.py"
    )
...
def test_detection_and_appearance(old_base, new_base) -> dict[str, float]:
    ...
    restored = pickle.loads(
        pickle.dumps(new_detections[1], protocol=pickle.HIGHEST_PROTOCOL)
    )
...
def make_observation(module, frame: int, logical_id: int) -> Any:
    ...
    velocity = np.array(
        [0.08 + logical_id * 0.001, np.cos(angle) * 0.03], dtype=np.float64
    )
    keypoints = np.stack(
...
def test_cluster_context_cache(old_base, new_base) -> None:
    ...
    class Assigner:
        ...
        def _prediction(track: Any, frame: int) -> np.ndarray:
            ...
            return (
                ...
                + np.asarray(track.velocity_px_per_frame, dtype=np.float64) * min(dt, 12)
            )
    ...
    old_context = old_manager.build_context(
        Assigner(tracks_for(old_base)), [], 10, (240, 320)
    )
    new_context = new_manager.build_context(
        Assigner(tracks_for(new_base)), [], 10, (240, 320)
    )
...
def test_pair_features(old_base, new_base, old_main, new_main) -> dict[str, float]:
    ...
    for _ in range(20):
        ...
        for i in range(20):
            for j in range(i + 1, 20):
                old_outputs.append(old_computer.compute(old_current[i], old_current[j], old_history, 1))
                old_outputs.append(old_computer.compute(old_current[j], old_current[i], old_history, 1))
    old_seconds = time.perf_counter() - started
    ...
    for _ in range(20):
        ...
        for i in range(20):
            for j in range(i + 1, 20):
                new_outputs.append(new_computer.compute(new_current[i], new_current[j], new_history, 1))
                new_outputs.append(new_computer.compute(new_current[j], new_current[i], new_history, 1))
    new_seconds = time.perf_counter() - started
    ...
    for i in range(20):
        for j in range(i + 1, 20):
            assert_same_value(
                old_computer.compute(old_history.previous(i), old_history.previous(j), old_history, 1),
                new_computer.compute(new_history.previous(i), new_history.previous(j), new_history, 1),
...
def test_identity_cascade_equivalence(new_base) -> None:
    ...
    for logical_id, det in enumerate(detections):
        ...
        det.bbox_xyxy = np.asarray(det.bbox_xyxy, dtype=np.float64) + np.array([shift_x, 0.0, shift_x, 0.0])
...
def test_frame_records_and_sqlite(old_base, new_base, old_main, new_main) -> None:
    ...
    for index, (old_record, new_record) in enumerate(zip(old_records, new_records)):
        ...
        assert set(old_record).issubset(set(new_record)), f"legacy schema missing at frame_record[{index}]"
