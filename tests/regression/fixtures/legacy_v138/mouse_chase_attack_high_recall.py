...
版本：v1.24.0（PPT定义驱动的追逐/攻击四分类 + 跳墙排除；追踪模块保持v1.23不变）
...
class PairFeatureComputer:
    ...
    def _trajectory_features(
        ...
        actor_hist = history.get(actor_id)[-self.history_frames :]
        target_hist = history.get(target_id)[-self.history_frames :]
        actor_by_frame = {x.frame: x for x in actor_hist}
        target_by_frame = {x.frame: x for x in target_hist}
        ...
        corr = float(np.clip(safe_corr(da.reshape(-1), db.reshape(-1)), -1.0, 1.0))
        return corr, float(np.linalg.norm(da, axis=1).sum()), float(np.linalg.norm(db, axis=1).sum())
    ...
    def _distance_drop(
        ...
        return float(previous - current) if np.isfinite(previous) else 0.0
    ...
    def compute(
        ...
        behind = bool(
            finite_point(actor.center_cm)
            and finite_point(target.center_cm)
            and np.all(np.isfinite(target.heading))
            and np.dot(actor.center_cm - target.center_cm, target.heading) < 0
        )
...
def classify_video_four_label(
    ...
    # A high-energy attack can resemble a brief pursuit.  When an attack is
    # already present, require the chase evidence to cover substantial travel
    # for one stable pair before assigning the combined label.
    if chase_present and attack_present and config is not None:
...
def _compute_behavior_frame_records(
    ...
    if pair_compute_mode in {"numpy", "multiprocess"}:
        ...
        pair_iter = ((actor, target, None, None) for actor, target in combinations(behavior_observations, 2))
...
class PairDataFrameStore:
    ...
    def pair_keys(self) -> List[str]:
        # 排序保证缓存复算与正常推理的事件编号顺序可重复。
        return sorted(self.table["pair_key"].dropna().astype(str).unique().tolist())

    def read_pair(self, pair_key: str) -> pd.DataFrame:
        # 返回副本，避免后处理新增列时污染其他鼠对或后续重复读取。
        pair = self.table[self.table["pair_key"].astype(str) == str(pair_key)].copy()
        # 帧号排序保持时间滤波输入严格单调。
...
def reconcile_detected_event_pairs(
    ...
    for event_index, original_event in enumerate(events):
        ...
        # Replace all pair and role fields consistently.
        ...
        event["pair_reconciliation_reason"] = (
            "event_level_evidence_consensus"
        )
...
def process_video(
    ...
    metadata = {
        ...
        "cluster_reid_ambiguous_count": int(cluster_reid.ambiguous_count),
        "behavior_definition_source": "RFID-CV 10鼠行为学分析0919.pptx",
        "behavior_classifier_version": "v1.24-document-defined",
...
def main(forced_stage: Optional[str] = None) -> int:
    ...
    try:
        config = load_yaml(resolve_runtime_path(args.config))
