...
def _reference_rolling_corr(
    ...
    for pair in range(pairs):
        ...
        for frame in range(frames):
            ...
            numerator = sample_count * float(np.dot(sample_a, sample_b)) - float(
                sample_a.sum()
            ) * float(sample_b.sum())
            variance_a = max(
                sample_count * float(np.dot(sample_a, sample_a)) - float(sample_a.sum()) ** 2,
                0.0,
            )
            variance_b = max(
                sample_count * float(np.dot(sample_b, sample_b)) - float(sample_b.sum()) ** 2,
                0.0,
            )
