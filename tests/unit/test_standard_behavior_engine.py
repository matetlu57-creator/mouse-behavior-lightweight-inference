...
def test_inactive_rows_are_skipped_without_changing_fsm_events():
    ...
    for level in ("weak", "strong"):
        ...
        ) == engine.extract_standard_behavior_events(full, 30.0, level, pair_key="1_2")
...
def test_invalid_timeline_gap_cannot_bridge_standard_fsm_events():
    ...
    assert all(not (event["start_frame"] < 18 and event["end_frame"] > 31) for event in events)
    ...
    df = pd.DataFrame(
        {
            ...
            "weak_standard_attack_subtype": [
                "lunge_attack" if 12 <= i <= 21 else "none" for i in frames
            ],
        }
    )
