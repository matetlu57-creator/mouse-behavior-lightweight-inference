...
def event_agreement(a: list[tuple[int, int, int, int, int]], b: list[tuple[int, int, int, int, int]]) -> float:
    if not a and not b:
        return 1.0
...
def main() -> int:
    ...
    print("TRACKING")
    print(f"  formal_id_count: {bt['formal_id_count']} -> {ot['formal_id_count']} | equal={id_count_equal}")
    print(f"  raw_to_logical_switch_count: {bt['raw_to_logical_switch_count']} -> {ot['raw_to_logical_switch_count']} | equal={switch_equal}")
