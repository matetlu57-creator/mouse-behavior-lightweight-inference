...
`Observation Quality -> Pair Kinematics -> Continuous Evidence -> Behavior-specific Role -> FSM/Hysteresis -> Independent Ethogram -> Four-class Label`
...
- 新增独立 Ethogram：`标准行为事件_时序引擎.csv`。追逐和攻击分别聚合，不因四分类标签切换而切断持续行为。
